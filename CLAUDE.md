...
- **Dev self-merge**: when `.claude/ci-status/pr-<N>.json` has matching SHA, `net_per_test > 0`, ratio <10%, no bucket >50 — run `gh pr merge <N> --auto` (queues for the merge queue; queue re-runs required checks on the merged state and lands the PR). Do NOT pass `--merge` (or any strategy flag) alongside `--auto`: the merge queue owns the strategy and rejects `--merge --auto`, which fails silently and leaves the PR green but never queued. Escalate to tech lead only when criteria fail. `--admin --merge` is reserved for workflow-only / hotfix bypass. See `.claude/skills/dev-self-merge.md`.
...
5. **On CI completion**:
   - **All required checks green** → run `/dev-self-merge`; if MERGE, `gh pr merge <N> --auto`, then proceed to step 8
