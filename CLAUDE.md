...
- **CRITICAL — fix push tracking after worktree creation**: `git worktree add -b <branch> origin/main` sets `branch.<name>.merge = refs/heads/main`, so `push.default=upstream` routes the first push to origin/main (not the new branch). Immediately after creating a worktree, fix the tracking: `cd /workspace/.claude/worktrees/<branch> && git branch --set-upstream-to=origin/<branch-name> <branch-name>`. Failure to do this causes accidental direct pushes to origin/main, bypassing PR + CI review.
