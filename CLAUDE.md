...
3. **Dev pushes the branch to origin and opens a PR against `main`** — PRs MUST target the **upstream** repo (`loopdive/js2`), never the fork (`ttraenkler/js2`). The checkout's gh default is pinned to upstream (`remote.upstream.gh-resolved=base`, shared by all worktrees), so plain `gh pr create` is correct; never pass `-R ttraenkler/js2`.
