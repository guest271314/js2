...
| **1851** | Make BackendEmitter an explicit legalization boundary + extract a declared type-converter; add a backend-neutral mid-level | Backlog | backlog | medium |
| **1852** | Make dynamic-value representation explicitly per-backend (typed refs / i31ref on WasmGC; f64-value + i32-tag on linear) | Backlog | backlog | medium |
