...
# Progress log (folded from the duplicate #3673 file)

The sections below were maintained in a second issue file that carried the
same id #3673 as this one (slug `acorn-performance`, added 2026-07-26 in
`7d82b897`). Two lanes filed the same problem independently, and the
duplicate id failed the required `check:issue-ids:against-main` gate. Its
content is folded in here and the duplicate deleted. Nothing is lost: the
`loc-budget-allow` grants it carried are preserved in this frontmatter, and
the round history is reproduced verbatim below.

## Measured root cause (host lane)
