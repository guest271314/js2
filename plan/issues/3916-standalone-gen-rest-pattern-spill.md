...
# #3916 — rest-in-binding-pattern generators leak the host generator machinery
...
`tests/issue-3916.test.ts` (24 cases) pins all of the above and documents the
...
- `scripts/claim-issue.mjs` could not write the claim ref during this work — its
  git subprocess exits 1 with empty stdout/stderr (`writeMode`, line 676). Both
  `--allocate` and a direct claim failed the same way; the id was allocated by
  hand against `origin/main` ∪ open-PR-added issue files ∪ the
  `origin/issue-assignments` ref (max reserved 3915). This is the #3880 breakage
  and it means the advisory lock gave **no** protection on this branch.
