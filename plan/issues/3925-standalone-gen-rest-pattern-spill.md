...
# #3925 — rest-in-binding-pattern generators leak the host generator machinery
...
`tests/issue-3925.test.ts` (24 cases) pins all of the above and documents the
...
all host-free and value-correct, and are pinned in `tests/issue-3925.test.ts`.
...
- **The broken allocator caused a real id collision on this very branch, and it
  is worth reading as evidence for #3880's priority.**
  ...
  **locally**. Another lane hand-allocated the *same* id concurrently, landed
  ...
  renumbered 3916 → 3925 as the loser.
- **Why 3925 and not 3922.** Max across all three sources was 3921, so every
  other lane hand-allocating "max + 1" while #3880 is down converges on 3922 —
  the single most collision-prone id available. The gap is deliberate: with the
  atomic reservation path unavailable, headroom is cheaper than a second
  renumber. Restore `--allocate` and this reasoning goes away.
