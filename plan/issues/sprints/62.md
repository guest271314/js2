...
## Carry-over from sprint 61 close (tech-lead wind-down)
...
- IR-TCO regression (task carried): experimental-IR re-lowers without tail calls; all 4 tests/tail-call-optimization.test.ts fail; evidence in plan/issues/1972-tail-call-inside-try-skips-catch.md Resolution section — file an issue first
...
## senior-fable handoff notes (2026-06-12 close)

- #1983 (name-collision refactor, task carried): the discriminator/helper work must ALSO cover the new `${Class}_init` names introduced by #1965's parent-init call rework (PR #1424).
- #1551: scope shrank after #1424 — super arg evaluation now flows through the real parent-init call; re-validate #1551's remaining repros against main once #1424 lands before resuming it.
