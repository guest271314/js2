...
|---|--------------------|-----------------------------|---------------------------------|--------|-------|--------|-----------|
| 1 | **IR `vec.get` element read** — `src/ir/from-ast.ts:1919` `lowerElementAccess` → `:1990` `emitVecGet` | index is always in `[0, length)`; **traps on OOB with NO SAFE fallback** (strictly worse than legacy, which returns a wrong sentinel) | index ∈ `[0, len)` via counted-loop bound or literal-index-below-known-length; **else** emit the SAFE bounds-checked read returning `undefined` | `partial` | easy (in-bounds half) / subtle (general dynamic index) | **M** | **#2766** (depends on #2760) |
