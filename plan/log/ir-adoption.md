...
| `type-resolution-failure`     | unintended | Same. **Dead/unreachable (#3341)**: nothing produces this reason — only the `IrFallbackReason` union decl in select.ts mentions it. Not promoted (vacuous + landmine if re-wired).                                                             |
