...
> This table is hand-maintained today. The intent is for a script under
> `scripts/` to regenerate it by inspecting `src/ir/from-ast.ts` once the
> file stabilises — tracked alongside #1530.
...
## Selector buckets (one row = one reason from `scripts/ir-fallback-baseline.json`)
...
When you finish an issue that moves a row, **edit it in the same PR**:

1. Find the kind row and update the Status column.
...
4. If you discovered a new rejection bucket, add a row to the Selector
   buckets table at the bottom.
