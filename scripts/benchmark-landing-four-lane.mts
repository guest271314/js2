...
function parseArguments(argv: readonly string[]): Arguments {
  ...
  if (validatePath && (argv.includes("--output") || withoutPorffor || canonicalUbuntu)) throw new Error(usage());
  ...
  return { outputDirectory, validatePath, withoutPorffor, canonicalUbuntu };
}
...
async function runProbe(options: Arguments): Promise<void> {
  ...
  const result: LandingFourLaneResult = {
    ...
    programs,
    cells: orderedCells as LandingSupportCell[],
    interpretation: {
      ...
      wasmtimeCompatibilityMethod:
        "exact source uses the existing landing options (wasi/nativeStrings/optimize=3), explicit experimentalIR:false compatibility fallback, Binaryen exact-reference normalization, then Wasmtime precompiled Cranelift execution",
    },
  };
  validateLandingFourLaneResult(result);
...
function compileAndLinkNative(
  ...
  for (const command of commands) {
    const result = timedSpawn(command[0], command.slice(1), cwd);
    wallMs += result.wallMs;
    if (result.status !== 0) {
      return { executable: null, commands, wallMs, diagnostic: commandFailureDiagnostic(`${mode}-clang`, result) };
    }
  }
  return { executable, commands, wallMs, diagnostic: null };
}
...
function usage(): string {
  return "usage: benchmark-landing-four-lane.mts [--probe] [--without-porffor] [--canonical-ubuntu] [--output <dir>] | --validate-result <json>";
}
