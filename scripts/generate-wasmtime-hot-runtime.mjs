...
function timeWasmtimeWarmIter(cwasmPath, arg, outerRuns) {
  const cmdArgs = landingWasmtimeRunArgs(cwasmPath, "warm", arg);
  const samplesMs = [];
...
async function main() {
  ...
  for (const program of LANDING_BENCHMARK_PROGRAMS) {
    ...
    const starlingMonkeyColdMs = timeWasmtimeFreshInstance(
      coldHostPath,
    ...
    rows.push(
      buildRow({
        ...
        starlingMonkeySamplesUs: toUs(starlingMonkeyColdMs),
      }),
    );
    rows.push(
      buildRow({
