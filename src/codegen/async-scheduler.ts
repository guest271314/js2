 ...
 */
function ensureUnhandledRejectionTracking(ctx: CodegenContext): void {
  const state = getOrInitState(ctx as CodegenContextWithScheduler);
  if (!ctx.wasi) return;
 ...
 */
function buildNoteUnhandledRejection(state: AsyncSchedulerState, promiseOnStack: Instr[]): Instr[] {
