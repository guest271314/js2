 ...
 */
/**
 * (#3468) Builtin/special function-value member names that must KEEP their
 ...
 * table (which would shadow the builtin). `.prototype` is intentionally absent:
 * a top-level `F.prototype = …` is already kept by the #2660 S2 arm (it
 * `continue`s before the #3468 keep is reached).
 */
  ...
  for (const stmt of sourceFile.statements) {
    ...
    if (ts.isExpressionStatement(stmt)) {
      ...
      if (ts.isBinaryExpression(expr)) {
        ...
        // (#3468) STANDALONE counterpart of the #2671 keep below (which is gated
        // `!ctx.standalone`): a top-level `F.<name> = …` static property write on
        // a top-level FUNCTION DECLARATION — the test262 assert-harness shape
        // (`assert.sameValue = function(){…}`, `assert._isSameValue = …`). `F` is
        // a function, not a module global, so the generic root-identifier check
        // below dropped the statement from `__module_init` under standalone: the
        // own property silently never existed, so `assert.sameValue(1,2)` invoked
        // `undefined` and every assertion was a VACUOUS PASS (#3468 root cause).
        // The SAME write already worked from inside a function body — only the
        // top-level collection dropped it — and the #3468 C-core closure-own-
        // property side table makes the ordinary `__extern_set` write-arm store
        // it on the function value. Keep it in `__module_init` so that arm runs.
        // Scoped exactly like the #2671 host arm: DIRECT bare-identifier receiver
        // only (`F.prototype.m = …` chains stay excluded — non-identifier
        // receiver); `.prototype` is already kept by the #2660 S2 arm above.
        // Host/GC is untouched (that lane uses the `!ctx.standalone` arm below),
        // so it stays byte-identical.
        if (
          ctx.standalone &&
          ts.isPropertyAccessExpression(expr.left) &&
          !ts.isPrivateIdentifier(expr.left.name) &&
          // Non-builtin, non-special property names only (the tech-lead's
          // exclusion surface). `.prototype` is already kept by the #2660 S2 arm
          // above (it `continue`d), so it never reaches here; the rest are
          // builtin function metadata/methods whose top-level write keeps its
          // existing (dropped, no-op) lowering to avoid shadowing the builtin.
