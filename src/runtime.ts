...
function _wrapWasmClosure(
  ...
  // (#1712 ⊕ main) union: main's `const wrapper` (required by the
  // byArity cache + return below) with this PR's `this: any` typing
  // (the body reads `this` for __call_fn_method_<arity> dispatch).
  const wrapper = function wasmClosureBridge(this: any, ...args: any[]): any {
