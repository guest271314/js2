...
`;
      ...
      const _resolveCtor = (thisArg: any): any => {
        ...
        // - Codegen emits `ref.null.extern` for the natural `Promise.all(iter)`
        //   call → default to global Promise.
        // - Otherwise treat as the explicit constructor and let the native
        //   engine validate (it throws TypeError if not a constructor, per
        //   NewPromiseCapability step 1).
        //
        // Caveat: an explicit `Promise.all.call(null, iter)` is observationally
        // indistinguishable from the natural call site here (both arrive as
        // null). Spec says the explicit form should throw TypeError, but
        // test262's `ctx-non-ctor` tests use truthy non-constructors ({}, 1,
        // Symbol(), …) — those flow through and native rejects correctly.
