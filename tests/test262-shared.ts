...
// (#3461) Fast native-harness oracle. Active ONLY when `TEST262_ORACLE_MODE=fast`
// AND the run is the HOST lane (`TEST262_TARGET` undefined — WasmGC + JS host).
// Standalone/linear/wasi cannot host-execute the harness (they forbid host
// imports), so they always use the honest whole-assembly v8 oracle regardless of
// the flag. Absent flag ⇒ honest path, byte-identical to today (AC#2).
const NATIVE_HARNESS_ORACLE = process.env.TEST262_ORACLE_MODE === "fast" && TEST262_TARGET === undefined;
